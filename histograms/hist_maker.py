...
def process_single_chunk(args_tuple):
    ...
    try:
        ...
        print(
            f"[CHUNK {chunk_index}] Using {len(chunk_seg_dict)} segmentation entries "
            f"for {len(chunk_files)} ROOT file(s)"
        )
                    ...
                    for var in vars_to_make_hist:
                        ...
                        if rdf_filtered is not None:
                            ...
                            if var not in available_columns:
                                print(
                                    f"[CHUNK {chunk_index}] WARNING: variable '{var}' not found. "
                            ...
                            if weight_name not in available_columns:
                                raise RuntimeError(
                                    f"Weight column '{weight_name}' not found "
                                    f"for systematic '{syst_name}'"
                                )
...
#         for f in chunk_files:
#             print(f"[CHUNK {chunk_index}]   {f}")
#         chunk_seg_dict = filter_seg_dict_for_files(full_seg_dict, chunk_files)

#         print(
#             f"[CHUNK {chunk_index}] Using {len(chunk_seg_dict)} segmentation entries "
#             f"for {len(chunk_files)} ROOT files"
#         )
#         rdf_base = GetRdfForDataset(
#             input_dir=args.input,
#             is_data=is_data,
