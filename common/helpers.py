...
# import math
# import ROOT, re, array, numpy as np, bisect
# import os
# import sys
# import json
...
# # =========================================================
# # Segmentation / denominator helpers
# # =========================================================
...
#     """
#     Return all JSON files associated with input_dir.

#     Supports both:
#     - input_dir as a directory
#     - input_dir as a single ROOT file
#     """

#     if input_dir.endswith(".root"):
#         search_dir = os.path.dirname(input_dir)
#     else:
...
# def get_segmentation_dict(input_dir, node="gen", root_files=None):
#     """
#     Build the denominator dictionary from JSON reports.

#     If root_files is provided, only JSON files with the same stem as
#     one of the selected ROOT files are included.

#     This is essential for chunked running:
#     - if a ROOT file is skipped,
#     - its JSON must also be skipped,
#     - otherwise inv_N_orig would include files not used in the histograms.
#     """
...
#         allowed_stems = {normalize_stem(f) for f in root_files}

#     json_files = get_json_files(input_dir)

#     for json_path in json_files:
#         json_stem = normalize_stem(json_path)

#         if allowed_stems is not None and json_stem not in allowed_stems:
#             continue
...
#             has_fine_segmentation = len(node_dict) > 1
...
#                 val = sum(float(v) for v in init_val.values())
#             else:
#                 val = float(init_val)
...
#     WARNING:
#     This only works if seg_dict keys are file-like names.
#     If seg_dict is already aggregated by selection, use get_segmentation_dict(..., root_files=...)
#     instead.
#     """
...
#     for weight_name_original, weight_info in weight_dict.items():
#         scales = ["central", "up", "down"] if store_shifted_weights else ["central"]

#         for scale in scales:
#             weight_name = weight_name_original

#             if weight_name == "Central" and scale != "central":
#                 continue
...
#                 weight_name = weight_name.format(scale=scale)
...
#             files_to_process = [explicit_files]
#         else:
#             files_to_process = list(explicit_files)
#     else:
#         files_to_process = get_root_files(input_dir)

#     if seg_dict is None:
#         seg_dict = get_segmentation_dict(
#             input_dir,
#             root_files=files_to_process,
#         )
...
#         if dims == 2:
#             return ROOT.RDF.TH2DModel("", "", *THModel_Inputs)

#         if dims == 3:
#             return ROOT.RDF.TH3DModel("", "", *THModel_Inputs)
...
#     new_hist = hist_initial.Rebin(
#         len(adapted) - 1,
#         sample,
#         array.array("d", adapted),
#     )
